...
def test_nested_prefixes_survive_sibling_branches_and_request_cleanup(backend):
    static_tokens, state_tokens = [5, 8, 13], [9, 6, 15, 16]
    static = backend.prefill(static_tokens)
    static_before = [v.clone() for v in cache_tensors(static.cache)]
    state = backend.prefill(state_tokens, static)
    state_before = [v.clone() for v in cache_tensors(state.cache)]
    for question in ([17, 18], [20, 21, 22]):
        parent = backend.prefill(question, state)
        actual = backend.score(parent, [[23], [24, 25, 26]])
        full = static_tokens + state_tokens + question
        expected = [backend.score(None, [full + row])[0] for row in ([23], [24, 25, 26])]
        torch.testing.assert_close(as_tensor(actual), as_tensor(expected), rtol=1e-5, atol=1e-5)
        backend.finish_request()
        backend.prepare_request([])
    for a, b in zip(static_before, cache_tensors(static.cache), strict=True):
        torch.testing.assert_close(a, b, rtol=0, atol=0)
    for a, b in zip(state_before, cache_tensors(state.cache), strict=True):
        torch.testing.assert_close(a, b, rtol=0, atol=0)
