...
def fork_cache(cache, size):
    """Batch independent copies of one parent; specialize known MLX cache layouts."""
    import mlx.core as mx
    from mlx_lm.models.cache import ArraysCache, BatchKVCache, KVCache

    if type(cache) is ArraysCache and cache.left_padding is None and cache.lengths is None:
        if all(c is None for c in cache.cache):
            return ArraysCache(len(cache.cache), left_padding=[0] * size)
        if all(c is not None and c.shape[0] == 1 for c in cache.cache):
            child = ArraysCache(len(cache.cache))
            child.cache = [mx.repeat(c, size, axis=0) for c in cache.cache]
            return child
    if type(cache) is KVCache and (cache.keys is None or cache.keys.shape[0] == 1):
        child = BatchKVCache([0] * size)
        if cache.keys is not None:
            child.keys = mx.repeat(cache.keys[..., : cache.offset, :], size, axis=0)
            child.values = mx.repeat(cache.values[..., : cache.offset, :], size, axis=0)
            child.offset = child.offset + cache.offset
            child._idx = cache.offset
        return child
    # Unknown/subclassed/rotating caches retain their own merge implementation
    # and defensive copies. Do not infer their state layout from shared names.
    return type(cache).merge([copy.deepcopy(cache) for _ in range(size)])
...
class MLXBackend:
    ...
    def __init__(self, config: ModelConfig):
        ...
        if config.vision:
            ...
            safe_types = {"qwen2", "qwen3", "qwen3_5", "qwen3_5_moe", "lfm2"}
            self.decoder = (
        ...
        if self.decoder is not None:
            # MLX's LFM2 always projects through its token embedding; its args
            # do not expose the tie_word_embeddings field used by Qwen.
            self.tied = (
                True
                if getattr(self.language_model, "model_type", None) == "lfm2"
                else self.language_model.args.tie_word_embeddings
            )
    ...
    def score(self, parent, suffixes):
        # Supported decoder scoring can gather each row's real verdict before right
        ...
        for group in groups.values():
            ...
            for start in range(0, len(group), size):
                ...
                if len(batch) > 1:
                    snapshot = MLXSnapshot(
                        [fork_cache(c, len(batch)) for c in seed.cache],
                        seed.length,
                        copy.deepcopy(seed.rope_delta),
                    )
                else:
