...
def compare(previous, current):
    ...
    pairs = []
    same_actions = same_scores = True
    for seed in protocol["seed_list"]:
        before, after = (records(root / f"game-{seed}-semif.jsonl") for root in (previous, current))
        same_actions &= [r["action"] for r in before] == [r["action"] for r in after]
        same_scores &= [r["probabilities"] for r in before] == [r["probabilities"] for r in after]
        pairs.append(
            {
                "seed": seed,
    ...
    result = {
        ...
        "semif_same_actions": same_actions,
        "semif_same_native_probabilities": same_scores,
    ...
    lines = [
        ...
        *["| " + " | ".join(row) + " |" for row in table],
        ...
        + ".",
        f"SemIf reproduced the same action sequences: {same_actions}. "
        f"Identical native choice distributions: {same_scores}.",
